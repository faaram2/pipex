...
/*   Updated: 2024/05/19 11:26:22 by ayeganya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	*result = 0;
	if (cache_extract(&ptr_cache, fd, result))
		return (result);
	extractor(fd, buf, &status);
	str_expander(buf, &result, &status);
	while (status == 1)
	{
		extractor(fd, buf, &status);
		str_expander(buf, &result, &status);
	}
	str_cache(buf, &result, &ptr_cache, fd);
	if (ft_strlen(result) == 0 || status == -1)
	{
		free(result);
		return (0);
	}
